...
$(function () {
  ...
  $('.button').on('click', function (event) {
    ...
    for (var _i = 0; _i < questions.length; _i++) {
      ...
      for (var _j = 0; _j < $('.question').eq(_i).children('.answer_list').children('li').length; _j++) {
        var userAnswersByNumber = $('.question').eq(_i).children('.answer_list').children('li').eq(_j).children('label').children('input').prop('checked');

        if (userAnswersByNumber !== rightAnswers[_i][_j]) {
